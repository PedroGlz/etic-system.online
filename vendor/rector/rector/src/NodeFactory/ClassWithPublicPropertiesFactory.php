...
use RectorPrefix20211020\Symplify\Astral\ValueObject\NodeBuilder\TraitUseBuilder;
/**
...
final class ClassWithPublicPropertiesFactory
{
    ...
    public function createNode(string $fullyQualifiedName, array $properties, ?string $parent = null, array $traits = [])
    {
        ...
        $classBuilder = new \RectorPrefix20211020\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder($className);
        if ($parent !== null && $parent !== '') {
            $classBuilder->extend($this->fixFullyQualifiedName($parent));
        }
        foreach ($traits as $trait) {
            $classBuilder->addStmt(new \RectorPrefix20211020\Symplify\Astral\ValueObject\NodeBuilder\TraitUseBuilder($this->fixFullyQualifiedName($trait)));
        }
        foreach ($properties as $propertyName => $propertySettings) {
            ...
            $propertyBuilder = new \RectorPrefix20211020\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder($propertyName);
