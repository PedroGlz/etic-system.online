...
use function substr;
use RectorPrefix20211020\SebastianBergmann\Diff\ConfigurationException;
 ...
 */
final class StrictUnifiedDiffOutputBuilder implements \RectorPrefix20211020\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if (!\is_bool($options['collapseRanges'])) {
            throw new \RectorPrefix20211020\SebastianBergmann\Diff\ConfigurationException('collapseRanges', 'a bool', $options['collapseRanges']);
        }
        if (!\is_int($options['contextLines']) || $options['contextLines'] < 0) {
            throw new \RectorPrefix20211020\SebastianBergmann\Diff\ConfigurationException('contextLines', 'an int >= 0', $options['contextLines']);
        }
        if (!\is_int($options['commonLineThreshold']) || $options['commonLineThreshold'] <= 0) {
            throw new \RectorPrefix20211020\SebastianBergmann\Diff\ConfigurationException('commonLineThreshold', 'an int > 0', $options['commonLineThreshold']);
        }
    ...
    private function assertString(array $options, string $option) : void
    {
        if (!\is_string($options[$option])) {
            throw new \RectorPrefix20211020\SebastianBergmann\Diff\ConfigurationException($option, 'a string', $options[$option]);
        }
    }
    private function assertStringOrNull(array $options, string $option) : void
    {
        if (null !== $options[$option] && !\is_string($options[$option])) {
            throw new \RectorPrefix20211020\SebastianBergmann\Diff\ConfigurationException($option, 'a string or <null>', $options[$option]);
        }
    }
}
