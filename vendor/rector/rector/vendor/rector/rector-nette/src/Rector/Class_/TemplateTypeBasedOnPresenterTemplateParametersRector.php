...
, [self::TEMPLATE_CLASS_PARENT => '', self::TEMPLATE_CLASS_TRAITS => []])]);
    ...
    public function configure(array $configuration) : void
    {
        if (isset($configuration[self::TEMPLATE_CLASS_PARENT])) {
            \RectorPrefix20211020\Webmozart\Assert\Assert::string($configuration[self::TEMPLATE_CLASS_PARENT]);
            $this->templateClassParent = $configuration[self::TEMPLATE_CLASS_PARENT];
        }
        if (isset($configuration[self::TEMPLATE_CLASS_TRAITS])) {
            \RectorPrefix20211020\Webmozart\Assert\Assert::isArray($configuration[self::TEMPLATE_CLASS_TRAITS]);
    ...
    private function createTemplateClass(string $presenterName, string $fullPresenterName, string $actionName, array $varTypes) : string
    {
        ...
        $fullTemplateClassName = '\\' . \RectorPrefix20211020\Nette\Utils\Strings::replace($fullPresenterName, $presenterPattern, $upperCasedActionName . 'Template');
