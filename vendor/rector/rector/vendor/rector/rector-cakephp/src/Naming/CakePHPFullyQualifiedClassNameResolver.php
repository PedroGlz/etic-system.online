...
final class CakePHPFullyQualifiedClassNameResolver
{
    ...
    public function resolveFromPseudoNamespaceAndShortClassName(string $pseudoNamespace, string $shortClass) : string
    {
        ...
        if (\RectorPrefix20211020\Nette\Utils\Strings::match($pseudoNamespace, self::LIB_NAMESPACE_PART_REGEX)) {
            $pseudoNamespace = \RectorPrefix20211020\Nette\Utils\Strings::replace($pseudoNamespace, '#\\\\Lib#', '');
        }
        ...
        if (\strpos($pseudoNamespace, '\\') !== \false && !\RectorPrefix20211020\Nette\Utils\Strings::match($pseudoNamespace, self::PLUGIN_OR_LIB_REGEX)) {
    ...
    {
        return \RectorPrefix20211020\Nette\Utils\Strings::replace($pseudoNamespace, self::SLASH_REGEX, '\\');
    }
}
