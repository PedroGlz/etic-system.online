...
use RectorPrefix20211020\Tracy\Dumper\Describer;
use RectorPrefix20211020\Tracy\Dumper\Exposer;
use RectorPrefix20211020\Tracy\Dumper\Renderer;
/**
 * Dumps a variable.
 */
class Dumper
{
    ...
    // deprecated
    public const HIDDEN_VALUE = \RectorPrefix20211020\Tracy\Dumper\Describer::HIDDEN_VALUE;
    /** @var Dumper\Value[] */
    ...
    /** @var array */
    public static $objectExporters = [\Closure::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeClosure'], \RectorPrefix20211020\UnitEnum::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeEnum'], \ArrayObject::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeArrayObject'], \SplFileInfo::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeSplFileInfo'], \SplObjectStorage::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeSplObjectStorage'], \__PHP_Incomplete_Class::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposePhpIncompleteClass'], \DOMNode::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeDOMNode'], \DOMNodeList::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeDOMNodeList'], \DOMNamedNodeMap::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeDOMNodeList'], \Ds\Collection::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeDsCollection'], \Ds\Map::class => [\RectorPrefix20211020\Tracy\Dumper\Exposer::class, 'exposeDsMap']];
    /** @var Describer */
    private $describer;
    /** @var Renderer */
    private $renderer;
    ...
    public static function dump($var, $options = [])
    {
        if (\PHP_SAPI === 'cli' || \PHP_SAPI === 'phpdbg') {
            $useColors = self::$terminalColors && \RectorPrefix20211020\Tracy\Helpers::detectColors();
    ...
    {
        static $sent;
        if (\RectorPrefix20211020\Tracy\Debugger::$productionMode === \true || $sent) {
            return;
        }
        $sent = \true;
        $nonce = \RectorPrefix20211020\Tracy\Helpers::getNonce();
        $nonceAttr = $nonce ? ' nonce="' . \RectorPrefix20211020\Tracy\Helpers::escapeHtml($nonce) . '"' : '';
        ...
        echo "<style{$nonceAttr}>", \str_replace('</', '<\\/', \RectorPrefix20211020\Tracy\Helpers::minifyCss($s)), "</style>\n";
        if (!\RectorPrefix20211020\Tracy\Debugger::isEnabled()) {
            $s = '(function(){' . \file_get_contents(__DIR__ . '/../Toggle/toggle.js') . '})();' . '(function(){' . \file_get_contents(__DIR__ . '/../Dumper/assets/dumper.js') . '})();';
            echo "<script{$nonceAttr}>", \str_replace(['<!--', '</s'], ['<\\!--', '<\\/s'], \RectorPrefix20211020\Tracy\Helpers::minifyJs($s)), "</script>\n";
        }
    }
    private function __construct(array $options = [])
    {
        ...
        $describer = $this->describer = new \RectorPrefix20211020\Tracy\Dumper\Describer();
        ...
        $renderer = $this->renderer = new \RectorPrefix20211020\Tracy\Dumper\Renderer();
    ...
    {
        $res = "'" . \RectorPrefix20211020\Tracy\Dumper\Renderer::jsonEncode($snapshot[0] ?? []) . "'";
        $snapshot = [];
        return $res;
    }
}
