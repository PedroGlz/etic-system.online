...
namespace RectorPrefix20211020\Symplify\ComposerJsonManipulator\Sorter;
...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority(string $requirementName) : string
    {
        if ($this->isPlatformPackage($requirementName)) {
            return \RectorPrefix20211020\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
                if ($name === 'php') {
                    return '0-' . $name;
                }
    ...
    {
        return (bool) \RectorPrefix20211020\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
