 ...
 */
class ResolveBindingsPass extends \RectorPrefix20211020\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process($container)
    {
        ...
        try {
            ...
            foreach ($this->unusedBindings as [$key, $serviceId, $bindingType, $file]) {
                ...
                if (\RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\BoundArgument::DEFAULTS_BINDING === $bindingType) {
                    $message .= 'under "_defaults"';
                } elseif (\RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\BoundArgument::INSTANCEOF_BINDING === $bindingType) {
                ...
                throw new \RectorPrefix20211020\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException($message);
            }
        } finally {
            $this->usedBindings = [];
            $this->unusedBindings = [];
    ...
    {
        if ($value instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\TypedReference && $value->getType() === (string) $value) {
            // Already checked
        ...
        if (!$value instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Definition || !($bindings = $value->getBindings())) {
            return parent::processValue($value, $isRoot);
        }
        $bindingNames = [];
        foreach ($bindings as $key => $binding) {
            ...
            if (null !== $bindingValue && !$bindingValue instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Reference && !$bindingValue instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Definition && !$bindingValue instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument && !$bindingValue instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                throw new \RectorPrefix20211020\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value for binding key "%s" for service "%s": expected "%s", "%s", "%s", "%s" or null, "%s" given.', $key, $this->currentId, \RectorPrefix20211020\Symfony\Component\DependencyInjection\Reference::class, \RectorPrefix20211020\Symfony\Component\DependencyInjection\Definition::class, \RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument::class, \RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument::class, \get_debug_type($bindingValue)));
            }
        }
        if ($value->isAbstract()) {
            return parent::processValue($value, $isRoot);
        ...
        foreach ($calls as $i => $call) {
            ...
            foreach ($reflectionMethod->getParameters() as $key => $parameter) {
                ...
                $typeHint = \RectorPrefix20211020\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter);
                $name = \RectorPrefix20211020\Symfony\Component\DependencyInjection\Attribute\Target::parseName($parameter);
     ...
     * @return mixed
     */
    private function getBindingValue(\RectorPrefix20211020\Symfony\Component\DependencyInjection\Argument\BoundArgument $binding)
    {
