 ...
 */
class DefinitionErrorExceptionPass extends \RectorPrefix20211020\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    /**
     * {@inheritdoc}
     * @param bool $isRoot
    ...
    {
        if (!$value instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Definition || !$value->hasErrors()) {
        ...
        if ($isRoot && !$value->isPublic()) {
            ...
            foreach ($graph->getNode($this->currentId)->getInEdges() as $edge) {
                if (!$edge->getValue() instanceof \RectorPrefix20211020\Symfony\Component\DependencyInjection\Reference || \RectorPrefix20211020\Symfony\Component\DependencyInjection\ContainerInterface::RUNTIME_EXCEPTION_ON_INVALID_REFERENCE !== $edge->getValue()->getInvalidBehavior()) {
                    $runtimeException = \false;
                    break;
                }
