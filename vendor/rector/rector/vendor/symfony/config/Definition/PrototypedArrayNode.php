...
use RectorPrefix20211020\Symfony\Component\Config\Definition\Exception\DuplicateKeyException;
 ...
 */
class PrototypedArrayNode extends \RectorPrefix20211020\Symfony\Component\Config\Definition\ArrayNode
{
    protected $prototype;
    ...
    {
        throw new \RectorPrefix20211020\Symfony\Component\Config\Definition\Exception\Exception('A prototyped array node can not have concrete children.');
    }
    /**
     * {@inheritdoc}
     */
    protected function finalizeValue($value)
    {
        ...
        if (\count($value) < $this->minNumberOfElements) {
            $ex = new \RectorPrefix20211020\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The path "%s" should have at least %d element(s) defined.', $this->getPath(), $this->minNumberOfElements));
        ...
        foreach ($value as $k => $v) {
            if (null !== $this->keyAttribute && \is_array($v)) {
                if (!isset($v[$this->keyAttribute]) && \is_int($k) && $isList) {
                    $ex = new \RectorPrefix20211020\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The attribute "%s" must be set for path "%s".', $this->keyAttribute, $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                    ...
                    if (\array_keys($v) === ['value']) {
                        $v = $v['value'];
                        if ($this->prototype instanceof \RectorPrefix20211020\Symfony\Component\Config\Definition\ArrayNode && ($children = $this->prototype->getChildren()) && \array_key_exists('value', $children)) {
                ...
                if (\array_key_exists($k, $normalized)) {
                    $ex = new \RectorPrefix20211020\Symfony\Component\Config\Definition\Exception\DuplicateKeyException(\sprintf('Duplicate key "%s" for path "%s".', $k, $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
            }
        ...
        foreach ($rightSide as $k => $v) {
            ...
            if (!\array_key_exists($k, $leftSide)) {
                if (!$this->allowNewKeys) {
                    $ex = new \RectorPrefix20211020\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('You are not allowed to define new elements for path "%s". Please define all elements for this path in one config file.', $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
                $leftSide[$k] = $v;
