 ...
 */
abstract class AbstractUnicodeString extends \RectorPrefix20211020\Symfony\Component\String\AbstractString
{
                ...
                if ('nfd' === ($rule = \strtolower($rule))) {
                    ...
                    if (null === ($transliterator = self::$transliterators[$rule] ?? (self::$transliterators[$rule] = \Transliterator::create($rule)))) {
                        ...
                        if (null === $transliterator) {
                            throw new \RectorPrefix20211020\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Unknown transliteration rule "%s".', $rule));
                        }
        ...
        if (\is_array($to) || $to instanceof \Closure) {
            ...
            $to = static function (array $m) use($to) : string {
                ...
                if ('' !== $to && (!\is_string($to) || !\preg_match('//u', $to))) {
                    throw new \RectorPrefix20211020\Symfony\Component\String\Exception\InvalidArgumentException('Replace callback must return a valid UTF-8 string.');
                }
                return $to;
            };
    ...
    public function snake() : parent
    {
        ...
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], 'RectorPrefix20211020\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
    /**
     * @param bool $allWords
    ...
    public function trim($chars = " \t\n\r\0\v\f ﻿") : parent
    {
        if (" \t\n\r\0\v\f ﻿" !== $chars && !\preg_match('//u', $chars)) {
            throw new \RectorPrefix20211020\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 chars.');
        }
    ...
    private function pad(int $len, self $pad, int $type) : parent
    {
        ...
        switch ($type) {
            ...
            default:
                throw new \RectorPrefix20211020\Symfony\Component\String\Exception\InvalidArgumentException('Invalid padding type.');
        }
    }
    /**
