...
final class Type
{
    ...
    public static function fromReflection($reflection) : ?self
    {
        ...
        if ($type === null) {
            ...
            throw new \RectorPrefix20211020\Nette\InvalidStateException('Unexpected type of ' . \RectorPrefix20211020\Nette\Utils\Reflection::toString($reflection));
        }
    }
    /**
    ...
    public static function fromString($type) : self
    {
        if (!\preg_match('#(?:
            ...
            throw new \RectorPrefix20211020\Nette\InvalidArgumentException("Invalid type '{$type}'.");
        }
        [, $nType, $iType] = $m;
        if ($nType) {
    ...
    {
        return $this->single && \RectorPrefix20211020\Nette\Utils\Reflection::isBuiltinType($this->types[0]);
    }
    /**
    ...
    public function allows($type) : bool
    {
        ...
        if ($this->isIntersection()) {
            ...
            return \RectorPrefix20211020\Nette\Utils\Arrays::every($this->types, function ($currentType) use($type) {
                $builtin = \RectorPrefix20211020\Nette\Utils\Reflection::isBuiltinType($currentType);
                return \RectorPrefix20211020\Nette\Utils\Arrays::some($type->types, function ($testedType) use($currentType, $builtin) {
                    return $builtin ? \strcasecmp($currentType, $testedType) === 0 : \is_a($testedType, $currentType, \true);
                });
            });
        }
        $method = $type->isIntersection() ? 'some' : 'every';
        return \RectorPrefix20211020\Nette\Utils\Arrays::$method($type->types, function ($testedType) {
            $builtin = \RectorPrefix20211020\Nette\Utils\Reflection::isBuiltinType($testedType);
            return \RectorPrefix20211020\Nette\Utils\Arrays::some($this->types, function ($currentType) use($testedType, $builtin) {
                return $builtin ? \strcasecmp($currentType, $testedType) === 0 : \is_a($testedType, $currentType, \true);
            });
        });
    }
